...
export const getResultBadge = (match: MatchResult): { label: string; cls: string } => {
  ...
  if (bocaGoals > rivalGoals) {
    return { label: `${bocaGoals}–${rivalGoals}`, cls: 'bg-status-win text-[#4ade80]' };
  }
  if (bocaGoals < rivalGoals) {
    return { label: `${bocaGoals}–${rivalGoals}`, cls: 'bg-status-loss text-[#fca5a5]' };
  }
  return { label: `${bocaGoals}–${rivalGoals}`, cls: 'bg-status-draw text-[#94a3b8]' };
};
...
export const useMiHistorial = (user: AuthUser) => {
  ...
  const availableCompetitions = [...new Set(
    attendedEntries
