...
export const MatchCombobox = ({
  ...
  const filtered = query.trim()
    ? matches.filter((m) =>
        formatOptionLabel(m).toLowerCase().includes(query.toLowerCase()),
      )
    : matches;
  ...
  return (
    <div ref={containerRef} className="relative flex-1">
      <div
        className={[
          "flex items-center bg-boca-blue border rounded-sm cursor-text",
          open
            ? "border-boca-gold"
            : value
              ? "border-boca-gold/60"
      ...
      >
        {open ? (
          ...
          <span
            className={[
              "flex-1 px-3 py-2.5 type-body truncate",
              value ? "text-text-nav" : "text-text-muted",
            ].join(" ")}
          >
            {value
              ? displayValue
              : matches.length === 0
                ? "Todos los partidos ya registrados"
        ...
        <div className="pr-2.5 shrink-0">
          <svg
            ...
            className={[
              ...
              open ? "rotate-180 text-boca-gold" : "text-text-muted",
            ].join(" ")}
          >
            <path
              d="M1 1l4 4 4-4"
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full mt-1 bg-boca-blue border border-boca-border rounded-sm shadow-lg z-50 max-h-56 overflow-y-auto">
          {filtered.length === 0 ? (
            <p className="px-3 py-2.5 type-body text-text-muted/70 italic">
              Sin resultados
            </p>
          ) : (
            filtered.map((match) => (
