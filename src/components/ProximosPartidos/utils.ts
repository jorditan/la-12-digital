...
export const RESULT_CFG = {
  win:  { bg: 'bg-status-win-subtle', border: 'border-status-win/25', text: 'text-status-win', dot: 'bg-status-win', label: 'G' },
  draw: { bg: 'bg-status-draw-subtle', border: 'border-status-draw/25', text: 'text-status-draw', dot: 'bg-status-draw', label: 'E' },
  loss: { bg: 'bg-status-loss-subtle', border: 'border-status-loss/25', text: 'text-status-loss', dot: 'bg-status-loss', label: 'P' },
} as const;
...
export function isFinishedH2HResult(
  ...
  return match.result === 'win' || match.result === 'draw' || match.result === 'loss';
}
