...
function getTrofeoDelDia(): TrofeoType {
  ...
  const diaDelAnio = Math.floor(
    (now.getTime() - inicio.getTime()) / 86_400_000,
  );
...
export function BannerMensaje() {
  ...
  return (
    <section aria-label="Banner Boca Juniors">
      ...
      <div className="border-t border-b border-boca-gold/15 py-3 px-6 flex items-center gap-4 bg-boca-blue-light ">
        {/* Palmarés del día — rota diariamente (Figma node 61:901) */}
        <TrofeoIcon type={trofeoHoy} />

        {/* Divisor vertical sutil */}
        <span
          className="h-4 w-px bg-boca-gold/25 shrink-0"
          aria-hidden="true"
        />

        <p className="font-serif text-base text-text-primary">{mensajeHoy}</p>
      </div>
    </section>
  );
}
