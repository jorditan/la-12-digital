...
const POSICION_ORDER: PosicionBucket[] = [
  "Delantero",
  "Mediocampista",
  "Defensor",
  "Arquero",
...
function normalizePosicion(posicion: string): PosicionBucket {
  ...
  if (p.includes("arquero") || p.includes("portero") || p.includes("golero"))
    return "Arquero";
  if (
    p.includes("delantero") ||
    p.includes("centrodelantero") ||
    p.includes("extremo") ||
    p.includes("punta")
  )
    return "Delantero";
...
export function FormacionPitch({ players, gameState }: FormacionPitchProps) {
  ...
  return (
    <div className="space-y-3">
      {/* Cancha */}
      <div className="relative rounded-sm overflow-hidden bg-pitch-field">
        <PitchLines />

        <div className="relative z-10 flex flex-col gap-5 py-5 px-3">
          {POSICION_ORDER.map((pos) => {
            const grupo = titulares.filter(
              (p) => normalizePosicion(p.posicion) === pos,
            );
