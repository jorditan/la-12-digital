...
export function BomboneraWidget() {
  const { proximoLocal, proximosLocales, diasHastaPartido, matchForecast, matchDayMode } = useBomboneraWidget();

  if (matchDayMode && proximoLocal) {
    return <ModoMatchDay proximoLocal={proximoLocal} matchForecast={matchForecast} />;
  }
