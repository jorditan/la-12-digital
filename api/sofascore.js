  ...
  try {
    const response = await fetch(url, {
      headers: {
        ...
        'Accept': isImageRequest ? 'image/webp,image/apng,image/*,*/*;q=0.8' : 'application/json, */*',
        'Accept-Language': 'es-AR,es;q=0.9,en-US;q=0.8,en;q=0.7',
        'Referer': 'https://www.sofascore.com/',
        'Origin': 'https://www.sofascore.com',
        'Cache-Control': 'no-cache',
        ...(isImageRequest && { 'Sec-Fetch-Dest': 'image', 'Sec-Fetch-Mode': 'no-cors', 'Sec-Fetch-Site': 'same-origin' }),
      },
    });
    ...
    if (contentType.includes('image') || contentType.includes('svg')) {
      ...
      if (!response.ok) { res.status(200).json(EMPTY_RESPONSE); return; }
